...
bool PolicerOrch::increaseRefCount(const string &name)
{
    ...
    SWSS_LOG_NOTICE("Policer %s reference count is increased to %d",
...
bool PolicerOrch::decreaseRefCount(const string &name)
{
    ...
    SWSS_LOG_NOTICE("Policer %s reference count is decreased to %d",
            name.c_str(), m_policerRefCounts[name]);
    return true;
}
    ...
    while (it != consumer.m_toSync.end())
    {
        ...
        if (op == SET_COMMAND)
        {
                    ...
                    i != kfvFieldsValues(tuple).end(); ++i)
            {
                ...
                SWSS_LOG_INFO("attribute: %s value: %s", field.c_str(), value.c_str());
